...
def describe_automaton(transitions):
    ...
    for k,v in transitions.items():
        print(f'When in state "{k[0]}" and processing "{k[1]}", automaton\'s state becomes "{v}".')
...
def accepts(transitions, word, initial_state, accept_state):
    ...
    for w in word:
        try:
            current = transitions[(current,int(w))]
        except:
            return False
...
for m in word:
    if (current_state, int(m)) in transistions:
        current_state=transitions[current_state, int(m)]
        else:
            return False
